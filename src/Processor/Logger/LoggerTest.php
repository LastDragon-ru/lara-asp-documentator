...
use LastDragon_ru\LaraASP\Documentator\Processor\Logger\Defaults\Formatter;
use LastDragon_ru\LaraASP\Documentator\Processor\Logger\Defaults\Output;
...
final class LoggerTest extends TestCase {
    ...
    public function testInvoke(string $expected, string $source): void {
        ...
        $listener  = new class($output, $formatter) extends Logger {
            public float   $time   = 0;
        ...
        foreach ($events as [$time, $event]) {
            if ($event instanceof ProcessBegin) {
                $listener->memory = new LoggerTest__Memory(0, 0);
            }

            if ($event instanceof ProcessEnd) {
                $listener->memory = new LoggerTest__Memory(0, 8 * 1024 * 1024);
            }
 ...
 */
readonly class LoggerTest__Memory extends Memory {
